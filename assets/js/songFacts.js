const songFacts = [
  {
    fact: " After Brooks & Dunn took 'Neon Moon' to No.1 in 1992, you were hard-pressed to walk into any honky tonk and not see someone line dancing to this absolute hit. Drowning your heartbreak under a tacky neon bar sign never sounded so good.",
    decade: "1990's",
    genre: "Country",
  },

  {
    fact: "Shania Twain had this hit twenty-five years later, this hit still gives people goosebumps when they hear the phrase “Let’s go girls.” The singer claims “It’s everyone’s song and they all take ownership of feeling comfortable in their own skin in a different way.”",
    decade: "1990's",
    genre: "Country",
  },

  {
    fact: "Wu-tang Clan’s 'Cream' was one of the most streamed rap songs out of the 90’s having close to 500 million streams since the original release date, also the beat was produced by the leader RZA who eventually left the group.",
    decade: "1990's",
    genre: "Rap",
  },

  {
    fact: "'Big Poppa' was seen as being one of Biggie Smalls biggest songs, he said in an interview that this song reflected his struggles obtaining money to leave the hood scene behind and to him what it means to be truly successful.",
    decade: "1990's",
    genre: "Rap",
  },

  {
    fact: "Nu metal became popular in the late 1990s, with bands and artists such as Korn, Limp Bizkit, and Slipknot all releasing albums that sold millions of copies.",
    decade: "1990's",
    genre: "Metal",
  },

  {
    fact: "Alternative Rock and it's sub-genres Grunge and Pop Punk expand in popularity and ironically, explode into the mainstream during the 1990's.",
    decade: "1990's",
    genre: "Rock",
  },

  {
    fact: "'Lose Yourself' was the inaugural winner of the Grammy Award for best rap song in 2004 and became the first rap song ever to be nominated for song of the year, celebrating it’s excellence in lyricism & also the first rap song to win an Oscar.",
    decade: "2000's",
    genre: "Rap",
  },

  {
    fact: "'Toxic' brought Britney Spears back to the pop top 10 for the first time in four years when it looked like her success was fading. The song hooks a listener the first time through and never lets go.",
    decade: "2000's",
    genre: "Pop",
  },

  {
    fact: "Upon its release, 'Hey Ya!' by Outkast was the rap-funk-pop behemoth reigned at No. 1 on the Hot 100 for nine weeks and became the first song ever to be downloaded 1 million times on iTunes.",
    decade: "2000's",
    genre: "Hip Hop/ R&B",
  },

  {
    fact: "Rihanna collaborated with Jay-Z on ‘Umbrella’ which wass nominated for Record of the Year and Song of the Year Grammy Awards. It spent 10 weeks at the top of the U.K. pop singles chart, the longest of any single in the decade.",
    decade: "2000's",
    genre: "Hip Hop/ R&B",
  },

  {
    fact: "It's an astounding accomplishment to record 30 No. 1 songs but beyond that, Luke Bryan … changed the course of country music. He gave his heart, his lifestyle, his musical infusion, powerful charisma, and Elvis-like moves, & forever changed the sound and reached for a new era in country music.",
    decade: "2000's",
    genre: "Country",
  },

  {
    fact: "If it seems like ‘I Hope You Dance’ has been around forever, it's because Lee Ann Womack's biggest hit is timeless. This song is about taking chances and not being afraid to try something new.",
    decade: "2000's",
    genre: "Country",
  },

  {
    fact: "Dierks Bentley burst onto the country music scene in 2003 when his self-titled debut album's toe-tapping first single climbed to the top of the Billboard country charts.",
    decade: "2000's",
    genre: "Country",
  },

  {
    fact: "Florida Georgia Line’s insanely catchy tune ‘Cruise’ spent 24 weeks at number one on the country chart and is the best-selling country digital song of all time in the US, and is generally considered the most famous example of the genre termed 'bro-country'.",
    decade: "2010's",
    genre: "Country",
  },

 {
    fact: "Everything about the pop genre changed when Taylor Swift came sniffing around in 2014, by the time she’d conceptualized the ’80s revisionist theme of 1989, there was already a working retro-themed pop apparatus in place spawning real hits.",
    decade: "2010's",
    genre: "Pop",
  },

  {
    fact: "Pop-stars ruled the radio stations, focusing their lyrics on drunken nights out with friends, the highlights of love and vengeful heartbreak.",
    decade: "2010's",
    genre: "Pop",
  },

  {
    fact: "Mainstream rock since the early 2010s had become softer and more refined because of indie rock. Rock was reportedly still the most widely consumed genre in the United States during 2014 and 2015.",
    decade: "2010's",
    genre: "Rock",
  },

  {
    fact: "The decade saw Foo Fighters win the Grammy Award for Best Rock Album a record-breaking three times; in 2001, 2004, and 2008.",
    decade: "2010's",
    genre: "Rock",
  },

  {
    fact: "Noted as one of the most prominent celebrities of the 21st century, Swift is recognized for her versatile musicality, songwriting prowess. ‘Anti-Hero’ quickly became Swift's biggest hit ever, reigning atop the Hot 100 for eight weeks and surpassing ‘Blank Space’ as her longest-running No. 1 song.",
    decade: "2020's",
    genre: "Pop",
  },

  {
    fact: "Morgan Wallen’s success can be attributed to his distinctive voice and relatable songwriting which set him apart from other country artists. His ability to connect with listeners on a personal level has resonated with fans and contributed to his success.",
    decade: "2020's",
    genre: "Country",
  },



];